...
- TweetDetail results are explicitly classified as available, explicitly unavailable, or absent. Tombstones are associated with a focal tweet through an exact result ID or a boundary-safe entry ID ending in `-tweet-<id>` when the result has no explicit ID; a conflicting explicit ID always wins. Three consecutive genuinely absent focal results stop a worker as a response-shape failure. Thread expansion persists positively associated tombstones but leaves absent targets retryable for a future invocation.
