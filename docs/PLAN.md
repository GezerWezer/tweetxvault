...
- Shipped import behavior: if auth is available, `tweetxvault import x-archive ...` runs bulk `tweets` / `likes` reconciliation and then processes one stable snapshot of all sparse `TweetDetail` rows eligible at the start of that phase. `--no-enrich` skips the per-item phase, `--detail-lookups N` bounds one import invocation, and bare `tweetxvault import enrich` selects every currently eligible row without an internal page cap while retaining 100-attempt write flushes and rate-limit pacing.
...
- TweetDetail results are explicitly classified as available, explicitly unavailable, or absent. Tombstones are associated with a focal tweet through an exact result ID or a boundary-safe entry ID ending in `-tweet-<id>` when the result has no explicit ID; a conflicting explicit ID always wins. A live-observed exact focal `TimelineTweet` entry containing `tweet_results: {}` is an explicit, retryable `unavailable_unknown` sentinel, but missing focal entries and nonempty malformed result containers remain absent. Three consecutive genuinely absent focal results stop a worker as a response-shape failure. Thread expansion persists positively associated tombstones but leaves absent targets retryable for a future invocation.
...
- Current SQLite schema-v4 databases open through a version-only fast path with no integrity scan,
  index setup, or repair work. New databases and LanceDB destinations create the latest schema
  directly. Existing pre-v3 archives take one additive direct-to-v4 migration with a validated
  atomic backup, unknown-reason preservation, available-row scheduler cleanup, and bounded indexed
  tombstone repair. Schema-v3 archives receive a transactional derived-index migration only: the
  FTS index is rebuilt from saved tweet memberships and the selective attachment index is added
  without copying unchanged canonical archive data. `db check` owns explicit integrity scans, while
  `repair legacy-tombstones --scan-timeline-captures` owns the optional expensive recovery scan.
...
- The Web presentation derives availability from canonical `tweet_object.enrichment_state` and `enrichment_reason`, using captured tombstones or recognizable legacy system text only when canonical state is incomplete. Feed rows are never removed for availability: direct posts and missing thread, quote, or retweet targets retain their position as typed placeholders, while `done`/`resurrected` content replaces stale tombstone payloads. Missing text is marked independently so retained media/actions remain usable, and raw Twitter media metadata supplies a non-download placeholder when older rows lack normalized media records.
