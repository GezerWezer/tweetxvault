...
- Current SQLite schema-v3 databases open through a version-only fast path with no integrity scan, index setup, or repair work. New databases and LanceDB destinations create the latest schema directly. Existing pre-v3 archives take one additive direct-to-v3 migration with a validated atomic backup, unknown-reason preservation, available-row scheduler cleanup, and bounded indexed tombstone repair. `db check` owns explicit integrity scans, while `repair legacy-tombstones --scan-timeline-captures` owns the optional expensive recovery scan.
